...
class StaphylococcusAureusMRSA < StaphylococcusAureus
  ...
  def increase_pathogenicity
    ...
    BioCatalogue::AVAILABLE_BACTERIAL_TREATMENTS.each { |treatment| @resistant_to << treatment unless @resistant_to.include?(treatment) }
  end
  ...
  def display_details
    ...
    str += line_short
    str += "    Virulence:\n"
    str += "    MRSA is one of the most dangerous modern infections. Usually hospital acquired it is resistant to nearly all known antibiotics and usually fatal.\n"
  end
end
